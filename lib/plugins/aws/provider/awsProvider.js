...
const impl = {
  ...
  /**
   * Add credentials, if present, from remote creds URL
   * @param results The results to add metadata credentials to
   */
  addRemoteCredentials: (results) => {
    const credentials = new AWS.RemoteCredentials({
      httpOptions: { timeout: 500 }, // .5 second timeout
      maxRetries: 0, // retry 0 times
    });
    console.log(credentials)
    impl.addCredentials(results, credentials);
  },
  /**
   * Add credentials, if present, from EC2 metadata
   * @param results The results to add metadata credentials to
   */
  addMetadataCredentials: (results) => {
...
class AwsProvider {
  ...
  getCredentials() {
    ...
    impl.addMetadataCredentials(result);
    impl.addRemoteCredentials(result);
    try {
